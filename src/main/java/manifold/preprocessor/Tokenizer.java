...
public class Tokenizer
{
  ...
  public Tokenizer( CharSequence buffer, int startOffset, int endOffset, Consumer<Tokenizer> consumer, Consumer<Problem>  issueConsumer)
  {
    _buffer = buffer;
  ...
  public void advance()
  {
    ...
    switch( c )
    {
      ...
      case '#':
        ...
        {
          int temp_index = _bufferIndex;
          makeSourceCodeToken( _bufferIndex + 1 );
          addError( "Invalid directive", temp_index ,getTokenEnd());
  ...
  private boolean matchDirective( int offset )
  {
    ...
    if( match( TokenType.If.getDirective(), offset ) )
    {
      ...
      _expr = new ExpressionParser( directive_start,_buffer, offset, _bufferEndOffset ,_issueConsumer).parse();
  ...
  private void addError( @SuppressWarnings("SameParameterValue") String message, int pos ,int end_pos)
  {

    if( _issueConsumer != null )
    {
      _issueConsumer.accept(  new Problem(message, pos,end_pos)  );
    }
    ...
    IDynamicJdk.instance().logError( Log.instance( JavacPlugin.instance().getContext() ), new JCDiagnostic.SimpleDiagnosticPosition( pos ),
