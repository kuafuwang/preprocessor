...
public class IfStatement extends Statement
{
  ...
  public void execute_for_no_visible(List<SourceStatement> result,boolean visible, Definitions definitions )
  {
    ...
    for( Statement stmt: _ifBlock )
    {
      stmt.execute_for_no_visible( result, ifCond, definitions );
    }
    ...
    for( IfStatement elif: _elifs )
    {
      boolean elifCond = visible && !elifPassed && !ifCond && elif.evalExpr( definitions );
      elifPassed = elifPassed || elifCond;
      elif.execute_for_no_visible( result, elifCond, definitions );
    }

    for( Statement stmt: _elseBlock )
    {
      stmt.execute_for_no_visible( result, visible && !ifCond && !elifPassed, definitions );
    }
